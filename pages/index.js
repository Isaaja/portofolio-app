...
const HomePage = () => {
    ...
    const animateGrayscale = () => {
      ...
      if (activeIndex < items.length) {
        ...
        setTimeout(() => {
          ...
          if (!isStillHover) {
            item.classList.remove("grayscale-0", "opacity-100", "scale-110");
            item.classList.add("grayscale-[0.6]", "opacity-90");

            if (icon) {
              icon.classList.remove("grayscale-0", "opacity-100", "scale-110");
  ...
  return (
    <>
      <Metadata title="Isa Iant Maulana | Web Developer & Data Analyst" />
      <HeroSection />
      <AboutSection />
      <SkillsSection />
